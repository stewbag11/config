(function () {
    ...
    function ConfigEditor(props) {
        ...
        const dialog =
            store.dialogMessage && store.dialogAction
                ? m$1(MessageBox, {
        ...
        return m$1(
            "div",
            {class: "config-editor"},
            props.header ? m$1("h3", {class: "sub-title"}, props.header) : null,
            m$1("textarea", {
                class: "editor",
            ...
            m$1("label", {class: "error-text"}, store.errorText),
            m$1(
                "div",
                {class: "buttons"},
                props.delete
                    ? m$1(Button, {onclick: showDeleteDialog}, "Delete")
                    : null,
                props.reset
                    ? m$1(Button, {onclick: showResetDialog}, "Reset changes")
                    : null,
                props.apply ? m$1(Button, {onclick: apply}, "Apply") : null
            ),
            m$1(
                "p",
                {class: "description"},
                ...
                m$1("strong", null, "support@darkreader.org")
            ),
            dialog
        );
    }
    ...
    function DynamicPerSiteEditor(props) {
        const context = getComponentContext();
        const store = context.getStore({
            errorText: "",
            fixes: [],
        ...
        if (store.currentFix && !store.fixes.includes(store.currentFix)) {
            ...
            const sameURLFix = store.fixes.find((f) => {
                const u2 = f.url;
                return (
                    u1.length === u2.length && u1.every((u, i) => u === u2[i])
                );
            });
            if (sameURLFix) {
        ...
        function addNewFix() {
            ...
            if (!newFixURL) {
                context.node.querySelector(".js-search").focus();
                return;
            }
            const newFix = {
                url: [newFixURL],
        ...
        const fixText = store.currentFix
            ? formatDynamicThemeFixes([store.currentFix])
            : "";
        const filteredFixes = store.search
            ? store.fixes.filter(({url}) =>
                  url.some((u) => u.includes(store.search))
              )
            : store.fixes;
        return m$1(
            "div",
            {class: "dynamic-per-site"},
            m$1(
                "div",
                {class: "dynamic-per-site__search-wrapper"},
                m$1(TextBox, {
                    class: "dynamic-per-site__search-input js-search",
                    type: "text",
                        ...
                        m$1(
                            Button,
                            {
                                class: {
                                    ...
                                    "dynamic-per-site__url--active":
                                        fix === store.currentFix
                                },
                                onclick: () => {
                                    store.currentFix =
                                        fix === store.currentFix ? null : fix;
                                    context.refresh();
                                }
                            },
                            text
            ...
            store.currentFix
                ...
                : m$1(
                      "div",
                      {class: "dynamic-per-site__add-fix"},
                      m$1(
                          Button,
                          {
                              onclick: addNewFix,
                              class: "dynamic-per-site__add-fix__button"
                          },
                          "Create new fix"
                      ),
                      m$1(
                          "p",
                          {class: "dynamic-per-site__add-fix__description"},
                          "Search for an ",
                          m$1("strong", null, "existing fix"),
                          " or enter a ",
                          m$1("strong", null, "domain name"),
                          m$1("br", null),
                          "for a new fix and click ",
                          m$1("strong", null, "Create")
                      )
                  )
        );
    }

    function DynamicModeEditor(props) {
        const context = getComponentContext();
        ...
        return m$1(
            ...
            m$1(
                ...
                m$1(
                    TabPanel$1.Tab,
                    {id: "full-editor", label: "Full Editor"},
                    m$1(ConfigEditor, {
                        text: props.devtools.dynamicFixesText,
                        apply: (text) =>
                            props.actions.applyDevDynamicThemeFixes(text),
                        reset: () => props.actions.resetDevDynamicThemeFixes()
                    })
                ),
                m$1(
                    TabPanel$1.Tab,
                    {id: "per-site-editor", label: "Per Site Editor"},
                    m$1(DynamicPerSiteEditor, {...props})
                )
            )
        );
    }
    ...
    function Body(props) {
        ...
        const previewButtonText = data.settings.previewNewDesign
            ...
            : "Preview new design";
        data.settings.previewNewestDesign
            ? "Switch to desktop design"
            : "Preview new mobile design";
        ...
        return m$1(
            ...
            m$1(
                TabPanel$1,
                {activeTabId: store.activeTabId, onTabChange: onTabChange},
                m$1(
                    TabPanel$1.Tab,
                    {id: "dynamic-editor", label: "Dynamic Theme Editor"},
                    m$1(DynamicModeEditor, {...props})
                ),
                m$1(
                    TabPanel$1.Tab,
                    {id: "static-editor", label: "Static Theme Editor"},
                    m$1(ConfigEditor, {
                ...
                m$1(
                    TabPanel$1.Tab,
                    {id: "filter-editor", label: "Inversion Fix Editor"},
                    m$1(ConfigEditor, {
    ...
    function renderBody(data, devToolsData, actions) {
        sync(
            document.body,
            m$1(Body, {data: data, devtools: devToolsData, actions: actions})
        );
    }
