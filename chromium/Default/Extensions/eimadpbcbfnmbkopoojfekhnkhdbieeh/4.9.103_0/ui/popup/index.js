(function () {
    ...
    async function getExtensionPageTabMV3() {
        return new Promise((resolve) => {
    ...
    async function openExtensionPage(page) {
        ...
        if (isMobile || page === "options") {
            const extensionPageTab = await getExtensionPageTab();
    ...
    function getSiteToggleMessage(props) {
        ...
        const tab = data.activeTab;
        const isFile = isLocalFile(tab.url);
        const isProtected = !isFile && tab.isProtected;
        return isProtected
    ...
    function Body(props) {
        ...
        const v = props.installation?.version
            ?.split(".")
            .map((p) => parseInt(p));
        ...
        const filterTab = m$1(
            ...
            m$1(
                ...
                m$1("span", {class: "birthday-message"}, birthdayMessageSpec),
                m$1(
                    "a",
                    {
                        class: "donate-link",
